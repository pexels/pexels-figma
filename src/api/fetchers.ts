...
const getKeyFactory = <K extends string, O extends {[k in K]?: any}>(
  name: string,
  listKey: K,
...
export const usePhotosSearch = (opts: Parameters<TPexelsClient['photos']['search']>[0]) => {
  ...
  const keyLoader = getKeyFactory<'photos', FetcherDataType>('photosSearch', 'photos', (...args: any[]) => [
    ...args,
    query.replace(/[^a-zA-Z_-]/g, '-'),
    orientation,
    color,
    size,
  ]);
